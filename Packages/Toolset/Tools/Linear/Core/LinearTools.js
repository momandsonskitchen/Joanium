...
async function linearGraphql(rootDirectory, query, variables = {}) {
  const credentials = await requireConnectorCredentials(rootDirectory, 'linear', ['token'], 'Linear');
...
export function createLinearToolHandlers({ rootDirectory }) {
  return {
    async linear_get_viewer() {
      const data = await linearGraphql(rootDirectory, 'query { viewer { id name email displayName url } }');
      const viewer = data.viewer;
      return [`Linear user: ${viewer.name || viewer.displayName}`, `Email: ${viewer.email || '(none)'}`, `URL: ${viewer.url || ''}`].join('\n');
    },

    async linear_list_teams(params = {}) {
      const limit = clampInteger(params.limit, 25, 1, 50);
      const data = await linearGraphql(rootDirectory, 'query($first:Int!){ teams(first:$first){ nodes { id key name description issueCount private } } }', { first: limit });
      return formatList('Linear teams', data.teams.nodes.map((team, index) => `${index + 1}. ${team.key} - ${team.name}\n   ID: ${team.id} | Issues: ${team.issueCount ?? 0}${team.private ? ' | private' : ''}`));
    },
    ...
    async linear_list_my_issues(params = {}) {
      ...
      if (state) issues = issues.filter((issue) => issue.state?.name?.toLowerCase() === state.toLowerCase());
    ...
    async linear_search_issues(params = {}) {
      ...
      return formatList(`Linear issue search: ${queryText}`, (data.issueSearch.nodes ?? []).map(formatIssue));
    },
    ...
    async linear_add_comment(params = {}) {
      ...
      const data = await linearGraphql(rootDirectory, 'mutation($input: CommentCreateInput!){ commentCreate(input:$input){ success comment { id url createdAt } } }', {
        input: { issueId, body }
      });
