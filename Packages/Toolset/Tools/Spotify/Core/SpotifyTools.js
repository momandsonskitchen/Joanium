...
async function spotifyRequest(rootDirectory, path, { searchParams = {} } = {}) {
  const credentials = await requireConnectorCredentials(rootDirectory, 'spotify', ['accessToken'], 'Spotify');
  ...
  if (!response.ok) throw new Error(`${response.status} ${response.statusText}: ${data?.error?.message ?? 'Spotify request failed'}`);
  return data;
}
...
export function createSpotifyToolHandlers({ rootDirectory }) {
  return {
    async spotify_get_profile() {
      ...
      return [`Spotify profile: ${profile.display_name || profile.id}`, `Email: ${profile.email ?? '(hidden)'}`, `Country: ${profile.country ?? 'unknown'}`, `Followers: ${profile.followers?.total ?? 0}`].join('\n');
    },
    ...
    async spotify_now_playing() {
      ...
      return [`Now playing`, trackLabel(data.item), `Progress: ${Math.round((data.progress_ms ?? 0) / 1000)}s`, `Playing: ${data.is_playing}`].join('\n');
    },
    ...
    async spotify_top_artists(params = {}) {
      ...
      return formatList('Spotify top artists', (data.items ?? []).map((artist, index) => `${index + 1}. ${artist.name}\n   Genres: ${(artist.genres ?? []).join(', ') || 'n/a'} | Followers: ${artist.followers?.total ?? 0}\n   ${artist.external_urls?.spotify ?? ''}`));
    },

    async spotify_list_playlists(params = {}) {
      const limit = clampInteger(params.limit, 10, 1, 25);
      const data = await spotifyRequest(rootDirectory, '/me/playlists', { searchParams: { limit } });
      return formatList('Spotify playlists', (data.items ?? []).map((playlist, index) => `${index + 1}. ${playlist.name}\n   Tracks: ${playlist.tracks?.total ?? 0} | Public: ${playlist.public ?? false}\n   ID: ${playlist.id}`));
    },

    async spotify_playlist_tracks(params = {}) {
      const playlistId = encodeURIComponent(requireText(params.playlist_id ?? params.playlistId, 'playlist_id'));
      const limit = clampInteger(params.limit, 20, 1, 50);
      const data = await spotifyRequest(rootDirectory, `/playlists/${playlistId}/tracks`, { searchParams: { limit } });
      return formatList('Spotify playlist tracks', (data.items ?? []).map((item, index) => trackLabel(item.track, index)));
    }
  };
}
