...
async function notionRequest(rootDirectory, path, { method = 'GET', body } = {}) {
  const credentials = await requireConnectorCredentials(rootDirectory, 'notion', ['token'], 'Notion');
...
function getTitle(value = {}) {
  ...
  if (value.object === 'page') {
    const titleProperty = Object.values(value.properties ?? {}).find((property) => property.type === 'title');
...
export function createNotionToolHandlers({ rootDirectory }) {
  return {
    ...
    async notion_get_page(params = {}) {
      const page = await notionRequest(rootDirectory, `/pages/${encodeURIComponent(requireText(params.page_id ?? params.pageId, 'page_id'))}`);
      const properties = Object.entries(page.properties ?? {}).map(([name, property]) => `${name}: ${property.type}`);
      return [formatResult(page), '', 'Properties:', ...properties].join('\n');
    },

    async notion_get_database(params = {}) {
      const database = await notionRequest(rootDirectory, `/databases/${encodeURIComponent(requireText(params.database_id ?? params.databaseId, 'database_id'))}`);
      const properties = Object.entries(database.properties ?? {}).map(([name, property]) => `${name}: ${property.type}`);
      return [formatResult(database), '', 'Properties:', ...properties].join('\n');
    },

    async notion_query_database(params = {}) {
      const databaseId = encodeURIComponent(requireText(params.database_id ?? params.databaseId, 'database_id'));
      const limit = clampInteger(params.limit, 10, 1, 25);
      ...
      const data = await notionRequest(rootDirectory, `/databases/${databaseId}/query`, { method: 'POST', body });
    ...
    async notion_create_page(params = {}) {
      ...
      const parentType = String(params.parent_type ?? params.parentType ?? 'page').trim().toLowerCase();
      ...
      return [`Notion page created`, `Title: ${getTitle(page)}`, `ID: ${page.id}`, `URL: ${page.url}`].join('\n');
    },

    async notion_append_block(params = {}) {
      const blockId = encodeURIComponent(requireText(params.block_id ?? params.blockId, 'block_id'));
