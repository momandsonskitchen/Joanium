const notionStrings = {
  connector: {
    id: 'notion',
    label: 'Notion',
    description: 'Search pages/databases, inspect records, query databases, and create simple pages through the Notion API.',
  ...
  tools: [
    {
      ...
      parameters: {
        ...
        filter: { type: 'string', required: false, description: 'page, database, or all. Defaults to all.' },
        limit: { type: 'number', required: false, description: 'Maximum results, default 10, max 25.' }
      }
    },
    {
    ...
    },
    {
      ...
      parameters: {
        database_id: { type: 'string', required: true, description: 'Notion database ID.' },
        filter: { type: 'string', required: false, description: 'Optional Notion filter JSON object.' },
        sorts: { type: 'string', required: false, description: 'Optional Notion sorts JSON array.' },
        limit: { type: 'number', required: false, description: 'Maximum rows, default 10, max 25.' }
      }
    },
    {
      ...
      category: 'notion',
      parameters: {
        parent_id: { type: 'string', required: true, description: 'Parent page ID or database ID.' },
        parent_type: { type: 'string', required: false, description: 'page or database. Defaults to page.' },
