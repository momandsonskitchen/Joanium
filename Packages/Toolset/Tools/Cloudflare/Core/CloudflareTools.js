...
async function cloudflareRequest(rootDirectory, path, { method = 'GET', searchParams = {}, body } = {}) {
  const credentials = await requireConnectorCredentials(rootDirectory, 'cloudflare', ['token'], 'Cloudflare');
  const url = new URL(`${CLOUDFLARE_API}${path}`);
  ...
  if (!response.ok || data?.success === false) {
    const message = data?.errors?.map((error) => error.message).join('; ') || 'Cloudflare request failed';
...
export function createCloudflareToolHandlers({ rootDirectory }) {
  return {
    async cloudflare_get_user() {
      ...
      return [`Cloudflare user: ${user.email}`, `ID: ${user.id}`, `Two factor: ${user.two_factor_authentication_enabled ?? false}`].join('\n');
    },
    ...
    async cloudflare_list_accounts(params = {}) {
      ...
      return formatList('Cloudflare accounts', accounts.map((account, index) => `${index + 1}. ${account.name}\n   ID: ${account.id}`));
    },
    ...
    async cloudflare_get_zone(params = {}) {
      return formatZone(await cloudflareRequest(rootDirectory, `/zones/${encodeURIComponent(requireText(params.zone_id ?? params.zoneId, 'zone_id'))}`));
    },
    ...
    async cloudflare_list_dns_records(params = {}) {
      ...
      return formatList(
        ...
        records.map((record, index) => `${index + 1}. ${record.type} ${record.name} -> ${record.content}\n   ID: ${record.id} | Proxied: ${record.proxied ?? false} | TTL: ${record.ttl}`)
      );
    },
    ...
    async cloudflare_create_dns_record(params = {}) {
      ...
      return [`Cloudflare DNS record created`, `${record.type} ${record.name} -> ${record.content}`, `ID: ${record.id}`].join('\n');
    },
    ...
    async cloudflare_purge_cache(params = {}) {
      ...
      return files.length ? `Purged ${files.length} Cloudflare cache URL(s).` : 'Purged the full Cloudflare zone cache.';
    }
  };
}
