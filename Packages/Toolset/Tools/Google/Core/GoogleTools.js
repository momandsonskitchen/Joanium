...
async function googleRequest(rootDirectory, url, { searchParams = {} } = {}) {
  ...
  if (!response.ok)
    throw new Error(
      `${response.status} ${response.statusText}: ${data?.error?.message ?? data?.error_description ?? 'Google request failed'}`,
    );
  return data;
}
...
export function createGoogleToolHandlers({ rootDirectory }) {
  return {
    async google_get_profile() {
      ...
      return [
        `Google profile: ${profile.name || profile.email}`,
        `Email: ${profile.email}`,
        `ID: ${profile.id}`,
      ].join('\n');
    },
    ...
    async drive_search_files(params = {}) {
      ...
      return formatList(
        `Google Drive search: ${params.query}`,
        (data.files ?? []).map(formatDriveFile),
      );
    },
    ...
    async calendar_get_upcoming(params = {}) {
      ...
      return formatList(
        ...
        (data.items ?? []).map(formatCalendarEvent),
      );
    },
